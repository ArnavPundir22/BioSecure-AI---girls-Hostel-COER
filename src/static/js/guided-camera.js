...
(function () {
  ...
  class GuidedCameraEngine {
    constructor(config = {}) {
      ...
      // AI Frame-skipping / Throttle parameters for smooth 60 FPS video preview
      this.lastScanTime = 0;
      this.isProcessingFrame = false;
      this.scanIntervalMs = 90; // Scan AI every ~90ms (~11 scans/sec) while UI renders at 60 FPS
    ...
    async startCamera() {
      try {
        ...
        // Determine if we are on a remote/non-local origin
        const hostname = location.hostname;
        const isLocalhost = hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '::1';
        // ngrok, localtunnel, and similar tunnels run over HTTPS — treat as secure
        const isTunnelDomain = /ngrok|loca\.lt|localtunnel|serveo|pagekite|cloudflare\.dev|trycloudflare/.test(hostname);
        const isRemoteIP = !isLocalhost;
        const isSecureCtx = window.isSecureContext &&
            navigator.mediaDevices &&
            typeof navigator.mediaDevices.getUserMedia === 'function';

        // Only block if we are remote AND not secure AND not a tunnel domain
        if (isRemoteIP && !isSecureCtx && !isTunnelDomain) {
          console.warn('GuidedCamera: Live camera stream requires HTTPS when accessing over IP on mobile browsers.');
          this.updateStatus('warning', 'HTTPS Required for Live Camera — Use Upload File tab', 'shield-alert');
          ...
          if (tabFileUpload) {
            setTimeout(() => {
              ...
              if (msg) {
                msg.className = 'p-4 mb-6 rounded-xl font-medium text-center shadow-md bg-amber-50 text-amber-800 border border-amber-300 text-xs';
                msg.innerHTML = `🔒 <strong>Browser Security Notice:</strong> Live camera on mobile requires <strong>HTTPS</strong> when accessing over IP (<code>${location.origin}</code>).<br>Switched to <strong>Upload File</strong> mode — tap the button to open your phone's camera!`;
        ...
        const isTunnelDomain = /ngrok|loca\.lt|localtunnel|serveo|pagekite|cloudflare\.dev|trycloudflare/.test(hostname);
        const isRemoteIP = !isLocalhost;
        if (isRemoteIP && !window.isSecureContext && !isTunnelDomain) {
          this.updateStatus('warning', 'HTTPS required for camera on remote IP', 'shield-alert');
        } else if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
          this.updateStatus('warning', 'Camera permission denied — allow camera access in browser', 'alert-triangle');
        } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
          this.updateStatus('warning', 'No camera found — use Upload File tab instead', 'alert-triangle');
        } else {
          this.updateStatus('warning', 'Camera unavailable — use Upload File tab', 'alert-triangle');
        }
      }
    }
    ...
    detectLoop() {
      ...
      if (!this.isProcessingFrame && (now - this.lastScanTime >= this.scanIntervalMs)) {
    ...
    updateStatus(type, text, iconName) {
      ...
      if (this.statusBadge) {
        ...
        if (type === 'success') {
          // Green badge — readable on dark video background
          this.statusBadge.classList.add('bg-emerald-500/30', 'text-emerald-100', 'border-emerald-500/40');
        } else if (type === 'warning') {
          this.statusBadge.classList.add('bg-rose-500/30', 'text-rose-100', 'border-rose-500/40');
        } else if (type === 'searching') {
          this.statusBadge.classList.add('bg-indigo-500/30', 'text-indigo-100', 'border-indigo-500/40');
        } else {
          this.statusBadge.classList.add('bg-black/50', 'text-white', 'border-white/20');
        }
      }
