...
def reload_face_cache() -> int:
    ...
    with _cache_lock:
        try:
            logger.info("Initializing/Reloading in-memory face embedding cache for girls_hostel...")
            try:
                response = supabase_admin.schema('girls_hostel').table('student_profiles') \
                    .select('id, name, roll_number, room_number, embedding') \
                    .execute()
                rows = response.data or []
            except Exception as schema_err:
                logger.warning(f"Girls Hostel schema query notice: {schema_err}. Initializing cache from hostel_db fallback.")
                from src.utils import hostel_db
                rows = hostel_db.fetch_all_hostel_students()
            if not rows:
                from src.utils import hostel_db
                rows = hostel_db.fetch_all_hostel_students()
            new_ids: List[str] = []
            ...
            for r in rows:
                ...
                try:
                    ...
                    if emb_arr.shape != (512,):
                        logger.warning(f"Embedding shape mismatch {emb_arr.shape} for student {sid}")
                        continue
...
def add_student_to_cache(
    ...
    with _cache_lock:
        ...
        _student_metadata[sid_clean] = {
            ...
            'roll_number': roll_number or branch,
